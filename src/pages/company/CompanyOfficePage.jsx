...
const CompanyOfficePage = () => {
  ...
  useEffect(() => {
    const fetchOffice = async () => {
      ...
      setOffice(response);
      setJobs(response.jobs || sampleJobs); // Use sampleJobs if no jobs are found
    };
    fetchOffice();
  }, [id]);
              ...
              <aside>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                  ...
                  <Link to={`/delete-office/${id}`}>
                    <Button className="bg-red-500 hover:bg-red-600 text-white text-center font-bold py-2 px-4 rounded-lg w-full focus:outline-none focus:shadow-outline mt-4">
                      Delete Office
                    </Button>
                  </Link>
                </div>
