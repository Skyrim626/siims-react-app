...
import { Briefcase, UsersRound, ClipboardCheck, BookText, CalendarClock, Calendar as CalendarIcon } from "lucide-react";
...
const SupervisorDashboardPage = () => {
  ...
  const recentInterns = [
    { id: 1, name: "Alice Johnson", jobTitle: "Web Developer", dateUpdated: "2024-11-26" },
    { id: 2, name: "John Smith", jobTitle: "UI/UX Designer", dateUpdated: "2024-11-25" },
    { id: 3, name: "Emily Davis", jobTitle: "Data Analyst", dateUpdated: "2024-11-24" },
  ];
  ...
  return (
    <Page>
      ...
      <Section>
        ...
        <Text className="text-sm text-blue-950">Overview of the system data.</Text>
        <hr className="my-3" />
      </Section>

      {/* Welcome Section */}
      <Section className="my-3 flex flex-col items-start">
        <Heading level={3}  text={`Welcome back, ${supervisorName || "Supervisor"}!`} className="font-bold text-gray-800 mt-2" />
      </Section>
      ...
      <Section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-lg shadow-lg flex items-center gap-4 transition-transform duration-200 hover:shadow-xl">
          ...
          <div>
            ...
            <Text className="text-sm px-2 text-gray-600">Active Internships</Text>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-lg flex items-center gap-4 transition-transform duration-200 hover:shadow-xl">
      ...
      <Section className="my-6">
        <Heading level={4} text={"Recent Interns Report"} className="text-gray-800 mb-4" />
            ...
            <tbody>
              {recentInterns.map((intern) => (
                <tr key={intern.id} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-300 px-4 py-2">{intern.jobTitle}</td>
                  <td className="border border-gray-300 px-4 py-2">{intern.dateUpdated}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center">
                    <div className="flex justify-center gap-2">
                      <button className="outline outline-2 outline-sky-800 bg-white text-sky-700 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-sky-200 transition"
                      ...
                      <button className="outline outline-2 outline-green-500 bg-white text-green-700 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-green-200 transition"
                      ...
                      <button
                        ...
                        onClick={() => alert(`Redirect to evaluation page for ${intern.name}`)}
                      >
